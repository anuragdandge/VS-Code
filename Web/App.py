...
# pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe' # For Windows (Update if needed)
pytesseract.pytesseract.tesseract_cmd = r'/usr/bin/tesseract'  # For Linux

app = Flask(__name__)
app.config['SECRET_KEY'] = 'secret!'  # Change this to a more secure key in production
socketio = SocketIO(app)
...
if not os.path.exists('output'):
    os.makedirs('output')
if not os.path.exists(os.path.join('output', 'duplicate')):
    os.makedirs(os.path.join('output', 'duplicate'))
...
def process_pdf(pdf_path, coordinates, zoom_factor=4, output_dir="output"):
    ...
    excel_path = os.path.join(output_dir, "pdf_processing_log.xlsx")
    duplicate_excel_path = os.path.join(output_dir, "duplicate_pdf_processing_log.xlsx")


    try:
        df = pd.read_excel(excel_path)
    except FileNotFoundError:
        df = pd.DataFrame(columns=["PDF File", "Company Code", "Farmer Name", "Timestamp", "Status", "Error Description","PDF Page No."])
    try:
        duplicate_df = pd.read_excel(duplicate_excel_path)
    except FileNotFoundError:
        duplicate_df = pd.DataFrame(columns=["PDF File", "Company Code", "Farmer Name", "Timestamp", "Status", "Error Description","PDF Page No."])

    pdf_document = fitz.open(pdf_path)
    processed_pdfs = {}  # Dictionary to store processed PDF names and counts
    ...
    for page_number in range(total_pages):
        socketio.emit('update', {'current': page_number + 1, 'total': total_pages, 'status': 'Processing'},
                      namespace='/')
        matched_text = None
        ...
        try:
            if matched_text:
                ...
                if pdf_name in processed_pdfs:
                    processed_pdfs[pdf_name]["count"] += 1
                    output_subdir = os.path.join(output_dir, "duplicate")

                    duplicate_df = pd.concat([duplicate_df, pd.DataFrame({"PDF File": pdf_path, "Company Code": pdf_name, "Farmer Name": farmer_name,
                                                                      "Timestamp": datetime.now(), "Status": "Duplicate Found", "PDF Page No.": page_number + 1}, index=[0])],
                                             ignore_index=True)
                else:
                    processed_pdfs[pdf_name] = {"count": 1, "pdf": fitz.open(), "farmer_name": farmer_name}
                    output_subdir = output_dir


                current_pdf = processed_pdfs[pdf_name]["pdf"]
                current_pdf.insert_pdf(pdf_document, from_page=page_number, to_page=page_number)
                processed_pdfs[pdf_name]["farmer_name"] = farmer_name  # Update farmer name


        except Exception as e:
            print(f"Error processing page {page_number + 1}: {e}")
            df = pd.concat([df, pd.DataFrame(
                {"PDF File": pdf_path, "Company Code": "N/A", "Farmer Name": farmer_name, "Timestamp": datetime.now(),
                 "Status": "Error", "Error Description": str(e),"PDF Page No.": page_number + 1}, index=[0])],
                           ignore_index=True)

    for pdf_name, pdf_data in processed_pdfs.items():
        current_pdf = pdf_data["pdf"]
        farmer_name = pdf_data["farmer_name"]
        if pdf_data["count"] > 1:
            output_subdir = os.path.join(output_dir, "duplicate")  # If count>1 save to duplicate
            status = "Duplicate Found" # Add status for excel logging of duplicates
        else:
            output_subdir = output_dir
            status = "Success"
        new_pdf_path = os.path.join(output_subdir, f"{pdf_name}.pdf")
        current_pdf.save(new_pdf_path)
        current_pdf.close()
        df = pd.concat([df, pd.DataFrame({"PDF File": pdf_path, "Company Code": pdf_name, "Farmer Name": farmer_name,
                                          "Timestamp": datetime.now(), "Status": status, "Error Description": "",
                                          "PDF Page No.": page_number + 1}, index=[0])], ignore_index=True)

    df.to_excel(excel_path, index=False)
    duplicate_df.to_excel(duplicate_excel_path, index=False)  # Save duplicate log

    socketio.emit('update', {'current': total_pages, 'total': total_pages, 'status': 'Complete',
                              'link': os.path.join('output', 'pdf_processing_log.xlsx'),
                              'duplicate_link': os.path.join('output', 'duplicate_pdf_processing_log.xlsx')},
                      namespace='/')
...
if __name__ == '__main__':
    socketio.run(app, debug=True, host='0.0.0.0') #Added host so that app is accessible on local network. Remove if not required
